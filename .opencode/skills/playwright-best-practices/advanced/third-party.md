...
export const test = base.extend<EmailFixtures>({
  getVerificationEmail: async ({ request }, use) => {
    await use(async (inbox) => {
      ...
      const response = await request.get(`https://api.mailinator.com/v2/domains/public/inboxes/${inbox}`, {
        headers: {
...
test.beforeEach(async ({ page }) => {
  ...
  await page.route(/google-analytics|googletagmanager|facebook|hotjar|segment|mixpanel|amplitude/, (route) =>
    route.abort(),
  );
});
```
