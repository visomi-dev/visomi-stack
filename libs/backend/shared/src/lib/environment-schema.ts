  ...
  .superRefine((data, context) => {
    ...
    if (data.ENABLE_TEST_API) {
      context.addIssue({
        ...
        message: 'Test APIs cannot be enabled in production.',
      });
    }
    if (
      data.SESSION_SECRET === 'themis-dev-session-secret' ||
      data.SESSION_SECRET.length < 32 ||
      /^(replace-|<)/.test(data.SESSION_SECRET)
    ) {
      context.addIssue({
        code: 'custom',
        path: ['SESSION_SECRET'],
    ...
    if (
      data.AUTH_TOTP_ENROLLMENT_ENABLED &&
      (data.AUTH_TOTP_ENCRYPTION_KEY.length < 32 || /^(replace-|<)/.test(data.AUTH_TOTP_ENCRYPTION_KEY))
    ) {
      context.addIssue({
        code: 'custom',
